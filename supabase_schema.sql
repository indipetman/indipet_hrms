...
CREATE TABLE public.employee_documents (
  ...
  verification_status character varying DEFAULT 'Not Verified'::character varying,
  verification_source character varying DEFAULT 'Manual portal check'::character varying,
...
CREATE TABLE public.employee_education (
  education_id integer NOT NULL DEFAULT nextval('employee_education_education_id_seq'::regclass),
  employee_id integer NOT NULL,
  qualification_level character varying NOT NULL,
  qualification_other character varying,
  course_name character varying,
  institution character varying,
  board_university character varying,
  passing_year smallint,
  grade_percentage character varying,
  education_status character varying DEFAULT 'Completed'::character varying,
  certificate_document_id integer,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT employee_education_pkey PRIMARY KEY (education_id),
  CONSTRAINT employee_education_employee_key UNIQUE (employee_id),
  CONSTRAINT employee_education_employee_id_fkey FOREIGN KEY (employee_id) REFERENCES public.employee_master(employee_id),
  CONSTRAINT employee_education_certificate_fkey FOREIGN KEY (certificate_document_id) REFERENCES public.employee_documents(document_id)
);
CREATE TABLE public.employee_experience (
  experience_id integer NOT NULL DEFAULT nextval('employee_experience_experience_id_seq'::regclass),
  employee_id integer NOT NULL,
  ...
  experience_document_id integer,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT employee_experience_pkey PRIMARY KEY (experience_id),
  CONSTRAINT employee_experience_employee_id_fkey FOREIGN KEY (employee_id) REFERENCES public.employee_master(employee_id),
  CONSTRAINT employee_experience_document_fkey FOREIGN KEY (experience_document_id) REFERENCES public.employee_documents(document_id)
);
...
CREATE TABLE public.employee_finance_benefits (
  finance_benefit_id integer NOT NULL DEFAULT nextval('employee_finance_benefits_finance_benefit_id_seq'::regclass),
  employee_id integer NOT NULL,
  bank_name character varying,
  branch_name character varying,
  account_number character varying,
  ifsc_code character varying,
  bank_verification_status character varying DEFAULT 'Not Verified'::character varying,
  pf_applicable boolean DEFAULT false,
  uan_number character varying,
  pf_member_id character varying,
  pf_status character varying DEFAULT 'Not Enrolled'::character varying,
  esi_applicable boolean DEFAULT false,
  esi_number character varying,
  esi_status character varying DEFAULT 'Not Enrolled'::character varying,
  nominee_name character varying,
  nominee_relationship character varying,
  status character varying DEFAULT 'Active'::character varying,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT employee_finance_benefits_pkey PRIMARY KEY (finance_benefit_id),
  CONSTRAINT employee_finance_benefits_employee_key UNIQUE (employee_id),
  CONSTRAINT employee_finance_benefits_uan_key UNIQUE (uan_number),
  CONSTRAINT employee_finance_benefits_esi_key UNIQUE (esi_number),
  CONSTRAINT fk_employee_finance_benefits_employee FOREIGN KEY (employee_id) REFERENCES public.employee_master(employee_id)
);
...
CREATE TABLE public.employee_family_members (
  family_member_id integer NOT NULL DEFAULT nextval('employee_family_members_family_member_id_seq'::regclass),
  employee_id integer NOT NULL,
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT employee_family_members_pkey PRIMARY KEY (family_member_id),
  CONSTRAINT employee_family_members_employee_id_fkey FOREIGN KEY (employee_id) REFERENCES public.employee_master(employee_id),
  CONSTRAINT employee_family_members_relationship_check CHECK (relationship::text = ANY (ARRAY['Father'::character varying, 'Mother'::character varying, 'Guardian'::character varying, 'Spouse'::character varying, 'Son'::character varying, 'Daughter'::character varying, 'Brother'::character varying, 'Sister'::character varying, 'Other'::character varying]::text[]))
);
...
CREATE TABLE public.employee_skills (
  ...
  years_experience numeric,
  certification_document_id integer,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT employee_skills_pkey PRIMARY KEY (skill_id),
  CONSTRAINT employee_skills_employee_name_key UNIQUE (employee_id, skill_name),
  CONSTRAINT fk_employee_skills_employee FOREIGN KEY (employee_id) REFERENCES public.employee_master(employee_id),
  CONSTRAINT employee_skills_certificate_fkey FOREIGN KEY (certification_document_id) REFERENCES public.employee_documents(document_id)
);
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    IF OLD.entity_role = 'Primary'
      AND EXISTS (
        SELECT 1
        FROM public.parent_entity
        WHERE entity_id <> OLD.entity_id
          AND entity_role = 'Franchisee'
      )
    THEN
      RAISE EXCEPTION 'Cannot delete the Primary Entity while Franchisee entities are registered under it.';
    END IF;
    RETURN OLD;
  END IF;
  ...
  IF NEW.entity_role = 'Primary'
    AND EXISTS (
      SELECT 1
      FROM public.parent_entity
      WHERE entity_id <> coalesce(NEW.entity_id, -1)
        AND entity_role = 'Primary'
    )
  THEN
    RAISE EXCEPTION 'Only one Primary Entity can exist in HRMS.';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'Create the Primary Entity first. Franchisee entities can be added after the Primary Entity exists.';
  END IF;
...
BEFORE INSERT OR UPDATE OF entity_role OR DELETE ON public.parent_entity
FOR EACH ROW
